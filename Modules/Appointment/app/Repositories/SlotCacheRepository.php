...
class SlotCacheRepository implements SlotRepositoryInterface
{
    protected SlotRepositoryInterface $repository;
    protected CacheRepository $cache;
    protected string $cacheKey = 'repo:slots';
    protected int $cacheExpiration = 60;

    public function __construct(SlotRepositoryInterface $repository, CacheRepository $cache)
    {
    ...
    {
        $cacheKey = "{$this->cacheKey}:filter:".json_encode($criteria);
        return $this->cache->tags([$this->cacheKey])->remember(
            $cacheKey,
            $this->cacheExpiration,
            function () use ($criteria, $columns) {
    ...
    {
        $cacheKey = "{$this->cacheKey}:available:{$doctorId}";

        return $this->cache->tags([$this->cacheKey])->remember(
            $cacheKey,
            $this->cacheExpiration,
            function () use ($doctorId) {
                return $this->repository->getAvailableSlotsByDoctor($doctorId);
            }
        );
    }
    ...
    {
        return $this->repository->findWithLock($id);
    }

    public function isReserved(int $id):bool
    {
        return $this->repository->isReserved($id);
    }
}
